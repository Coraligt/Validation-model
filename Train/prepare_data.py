...
def preprocess_dataset(data_dir, output_dir, train_indices_file, use_columns=['t', 'q']):
    ...
    # Save scalers in multiple locations for easier access
    ...
    try:
        ...
        print(f"Error saving scalers to {scalers_path}: {e}")
    
    # Also save to parent directory for easier access during inference
    parent_dir = os.path.dirname(output_dir)
    parent_scalers_path = os.path.join(parent_dir, 'scalers.pkl')
    try:
        with open(parent_scalers_path, 'wb') as f:
            pickle.dump(scalers, f)
        print(f"Also saved scalers to {parent_scalers_path} for easier access")
    except Exception as e:
        print(f"Error saving scalers to parent directory: {e}")
    ...
    # Verify that at least one scaler file was successfully saved
    if os.path.exists(scalers_path) or os.path.exists(parent_scalers_path):
        print(f"Scaler file(s) successfully saved.")
    else:
        print(f"WARNING: No scaler files were saved! Please check file permissions.")
